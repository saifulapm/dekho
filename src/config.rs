//! The optional config file, which today holds one thing: the TMDB key.
//!
//! The environment still wins. `TMDB_API_KEY` is how the tool was documented,
//! and it is how a one-off run overrides everything. The file exists because a
//! desktop panel spawns dekho without a login shell's exported environment, so
//! the key has to be readable from somewhere on disk too.
//!
